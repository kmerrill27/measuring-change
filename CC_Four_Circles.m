...
[centersA, radiiA] = imfindcircles(rgb, [20, 40], 'ObjectPolarity', 'bright');
[centersB, radiiB] = imfindcircles(rgb, [40, 60], 'ObjectPolarity', 'bright');

% Merge circles found in two ranges
allCenters = [centersA; centersB];
allRadii = [radiiA; radiiB];

if (size(allCenters, 1) < 4)
    disp('ColorChecker not found (A).');
    cropped = imcrop(rgb);
else

    % Select four strongest circles in image
    centers = allCenters(1:4,:);
    radii = allRadii(1:4);
...
for i=1:numSquares
    ...
    column = width;
    r = ySquareCoord;
    while (bwIm(r, column) < 0.5) && (r < height)
    ...
    xSect = (ceil((xSquareCoord - (width - column)) / widthBlock));
    ySect = (ceil((ySquareCoord - rowB) / heightBlock));
    if (xSect > 14 || ySect > 10)
        continue;
    end
    xySections = [xySections; xSect, ySect];
